...
from drp.data.dataset import Drp3dMMapDataset, Drp3dMMapDatasetSSL, Drp3dSqlite
...
class TestDataset(unittest.TestCase):
    ...
    def test_minicube(self):
        dataset = Drp3dMMapDataset(self.config, train_flag="train")
        minicube, permeability, cube_id = dataset[0]
        print(len(dataset))
        print(minicube.shape)

    def test_sqlite_dataset(self):
        dataset = Drp3dSqlite(self.config, train_flag="train")
        minicube, permeability, cube_id = dataset[0]
        print(len(dataset), permeability, cube_id)
        print(minicube.shape)

        dataset = Drp3dSqlite(self.config, train_flag="valid")
        minicube, permeability, cube_id = dataset[0]
        print(len(dataset), permeability, cube_id)
        print(minicube.shape)
